...
WHERE t.status = 'scheduled'
  ...
  AND ($3::int IS NULL OR t.provider_id = $3)
  AND ($4::int IS NULL OR b.bus_type_id = $4)
ORDER BY t.departure_time ASC
LIMIT $1 OFFSET $2
`

type BrowseUpcomingTripsParams struct {
...
type BrowseUpcomingTripsRow struct {
	ID              int64              `json:"id"`
	ProviderID      int32              `json:"providerId"`
...
func (q *Queries) BrowseUpcomingTrips(ctx context.Context, arg BrowseUpcomingTripsParams) ([]BrowseUpcomingTripsRow, error) {
	rows, err := q.db.Query(ctx, browseUpcomingTrips,
		arg.Limit,
		arg.Offset,
		arg.ProviderID,
		arg.BusTypeID,
	...
	defer rows.Close()
	items := []BrowseUpcomingTripsRow{}
	for rows.Next() {
		var i BrowseUpcomingTripsRow
		if err := rows.Scan(
			&i.ID,
			&i.ProviderID,
			&i.BusID,
...
const countBrowseUpcomingTrips = `-- name: CountBrowseUpcomingTrips :one
SELECT COUNT(*)
FROM trips t
JOIN buses b ON t.bus_id = b.id
WHERE t.status = 'scheduled'
  AND t.departure_time > NOW()
  AND t.available_seats > 0
  AND ($1::int IS NULL OR t.provider_id = $1)
  AND ($2::int IS NULL OR b.bus_type_id = $2)
`

type CountBrowseUpcomingTripsParams struct {
	ProviderID *int32 `json:"providerId"`
	BusTypeID  *int32 `json:"busTypeId"`
}

func (q *Queries) CountBrowseUpcomingTrips(ctx context.Context, arg CountBrowseUpcomingTripsParams) (int64, error) {
	row := q.db.QueryRow(ctx, countBrowseUpcomingTrips, arg.ProviderID, arg.BusTypeID)
	var count int64
	err := row.Scan(&count)
	return count, err
}
