...
export const createBusTypeSchema = z.object({
    ...
    totalSeats: z.coerce.number().min(1, V.minNum('field.totalSeats', 1)).max(100, V.maxNum('field.totalSeats', 100)),
    seatLayout: z.record(z.unknown()).optional().default({}),
})

export const updateBusTypeSchema = z.object({
    name: z.string().min(2, V.min('field.name', 2)).max(100, V.tooLong('field.name')),
    totalSeats: z.coerce.number().min(1, V.minNum('field.totalSeats', 1)).max(100, V.maxNum('field.totalSeats', 100)),
    seatLayout: z.record(z.unknown()).optional(),
})
